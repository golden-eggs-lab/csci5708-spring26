...
#include "utils/guc.h"		/* Lab 2: needed for DefineCustomBoolVariable */
#include "utils/index_selfuncs.h"
#include "utils/memutils.h"

/* ---------------------------------------------------------------------------
 * Lab 2: B-tree Instrumentation
 *
 * TODO (1 of 3) — Declare extern references and implement the GUC registration.
 *
 * (a) Add extern declarations for all four variables you defined in
 *     nbtsearch_template.c (the bool flag + three int counters).
 *
 * (b) Implement _bt_lab_register_guc() below.  It must:
 *     - Use DefineCustomBoolVariable() to register the GUC
 *       "btree_lab.enable_stats", pointing at your bool flag.
 *     - Call MarkGUCPrefixReserved("btree_lab") afterward.
 *     - Use a static bool to guard against double-registration.
 *
 * Hint: look up DefineCustomBoolVariable in src/include/utils/guc.h.
 * The GUC context should be PGC_USERSET; pass NULL for all hooks.
 * --------------------------------------------------------------------------- */

/* YOUR CODE HERE — extern declarations */
		 ...
		 IndexInfo *indexInfo)
{
	...
	/*
	 * TODO (2 of 3) — Instrument btinsert().
	 *
	 * (a) Call _bt_lab_register_guc() once (it self-guards).
	 * (b) Reset all three counters to 0.
	 * (c) After _bt_doinsert() returns (below), if stats are enabled,
	 *     emit one line with elog(INFO, ...) in the exact format:
	 *       "BTREE_LAB_STATS: internal=%d leaf=%d splits=%d"
	 *     passing the three counters.
	 */

	/* YOUR CODE HERE — (a) register GUC, (b) reset counters */
	...
	/* YOUR CODE HERE — (c) print stats if enabled */
...
btbeginscan(Relation rel, int nkeys, int norderbys)
{
	...
	/*
	 * TODO (3 of 3, part A) — Instrument btbeginscan().
	 *
	 * Register the GUC (if not already done) and reset all three
	 * counters so this scan's stats start from zero.
	 */
...
btendscan(IndexScanDesc scan)
{
	...
	/*
	 * TODO (3 of 3, part B) — Emit stats and reset at end of scan.
	 *
	 * If stats are enabled, print the counters with elog(INFO, ...)
	 * using the exact same format as in btinsert:
	 *   "BTREE_LAB_STATS: internal=%d leaf=%d splits=%d"
	 * Then reset all three counters to 0.
	 *
	 * Note: print BEFORE resetting, or you will always log zeros.
	 */

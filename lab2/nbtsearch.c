 ...
 *
 * TODO (1 of 2) — Define the instrumentation state.
 *
 * You need four file-scope variables here (all with external linkage):
 *   - A bool flag that controls whether stats collection is active.
 *     It will be wired to a GUC in nbtree_template.c.
 *   - Three int counters, all initialised to 0:
 *       bt_internal_visits  (internal pages visited during search)
 *       bt_leaf_visits      (leaf pages visited during search)
 *       bt_splits           (page splits during insert)
 *
 * Hint: think about which file increments bt_splits — it must be
 * reachable from there, so these cannot be static.
 * --------------------------------------------------------------------------- */
/* YOUR CODE HERE */
	...
	*bufP = _bt_getroot(rel, heaprel, access);
	...
	for (;;)
	{
		...
		*bufP = _bt_moveright(rel, heaprel, key, *bufP, (access == BT_WRITE),
		...
		{
			/*
			 * TODO (2 of 2, part A): We have reached a leaf page.
			 * If stats collection is enabled, increment the leaf-visit
			 * counter.  Then break out of the descent loop.
			 */
			/* YOUR CODE HERE */
			break;
		}
		...
		/*
		 * TODO (2 of 2, part B): This is an internal (non-leaf) page.
		 * If stats collection is enabled, increment the internal-visit
		 * counter before following the downlink to the child.
		 */
		/* YOUR CODE HERE */

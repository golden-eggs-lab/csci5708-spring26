...
/*
 * TODO: Declare the global timestamp counter for 2nd-LRU here.
 *
 * This should be a global integer variable, initialized to 1, that will be
 * incremented each time a buffer is pinned (the incrementing is handled in
 * bufmgr.c -- you just need to declare and initialize the variable here).
 *
 * Hint: It needs to be accessible from bufmgr.c via an "extern" declaration.
 */
...
StrategyGetBuffer(BufferAccessStrategy strategy, uint32 *buf_state, bool *from_ring)
{
	...
	*from_ring = false;
	...
	/*
	 * ============================================================
	 * TODO: Implement the 2nd-LRU buffer replacement algorithm.
	 * ============================================================
	 *
	 * At this point, the freelist is empty. You need to find a victim buffer
	 * using the 2nd-LRU policy.
	 *
	 * Requirements:
	 *
	 * 1. SCANNING: Scan ALL buffers (index 0 to NBuffers-1) to find
	 *    "candidate" buffers. A buffer is a candidate if its refcount is 0
	 *    (i.e., it is not currently pinned by any backend).
	 *
	 *    Useful constructs:
	 *      - GetBufferDescriptor(i)  --> returns BufferDesc* for buffer i
	 *      - pg_atomic_read_u32(&candidate->state)  --> reads buffer state
	 *      - BUF_STATE_GET_REFCOUNT(state)  --> extracts refcount from state
	 *      - candidate->lru_timestamp  --> the buffer's LRU timestamp
	 *
	 * 2. SELECTION: Among all candidates, find the one with the SECOND
	 *    smallest lru_timestamp. That is the victim to evict.
	 *    - If there is only 1 candidate, evict that one.
	 *    - If there are 0 candidates, retry (see step 5).
	 *
	 *    Hint: Track both the least and second-least timestamps (and their
	 *    buffer indices) as you scan. Initialize them to INT_MAX.
	 *
	 * 3. LOGGING (IMPORTANT — autograder will parse this output):
	 *    You MUST use the EXACT printf format shown below. Do NOT modify
	 *    the format strings. Copy this code verbatim into your implementation.
	 *
	 *    --- Print candidate list (after scanning, before evicting): ---
	 *
	 *        {
	 *            int first = 1;
	 ...
	 *        }
	 *
	 *    --- Print replaced buffer (immediately after candidate list): ---
	 *
	 *        printf("Replaced buffer: %d\n", buf->lru_timestamp);
	 *
	 *    (Where "buf" is your chosen victim BufferDesc*.)
	 *
	 * 4. LOCKING & RETURNING: Once you've chosen the victim:
	 *      local_buf_state = LockBufHdr(buf);
	 *    Then re-verify that refcount is still 0 (another backend might have
	 *    pinned it between your scan and your lock). If still 0:
	 *      - If strategy != NULL, call AddBufferToRing(strategy, buf);
	 *      - Set *buf_state = local_buf_state;
	 *      - return buf;   (with the spinlock still held!)
	 *    If refcount is no longer 0:
	 *      UnlockBufHdr(buf, local_buf_state);
	 ...
	 * implementation.
	 */
	elog(ERROR, "2nd-LRU replacement not yet implemented");
